...
//without authorization , 401 is showing
    ...
    public void testEditorDelete(){
        given()
                .auth().basic("editor","editor")//403: status code:you are not authorization
                .and().accept(ContentType.JSON)
                .and().pathParam("id",30)
        .when()
    ...
    /*
        As a homework,write a detailed test for Role Base Access Control(RBAC)
            ...
            User should be able to only READ data
                not update,delete,create (POST,PUT,PATCH,DELETE)  403 status code expect
       --------------------------------------------------------
