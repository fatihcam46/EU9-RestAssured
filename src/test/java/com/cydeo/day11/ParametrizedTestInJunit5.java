...
public class ParametrizedTestInJunit5 {
    ...
    public void testMultipleNumbers(int number){
        ...
        Assertions.assertTrue(number > 5);//hangi sayi 5 ten buyukse dogrula.
    ...
    @ParameterizedTest
    @ValueSource(ints = {22030,22031, 22032, 22033 , 22034, 22035, 22036})//7 tanenin her bririni kontrol edip 200 mu bakiyor
    public void zipCodeTest(int zipCode){

        given()
                .baseUri("https://api.zippopotam.us")
                .pathParam("zipcode",zipCode) //we got zipcode from valueSource//OUR Junit5 is used
                .log().all() //request log
        .when()
                .get("/us/{zipcode}")
