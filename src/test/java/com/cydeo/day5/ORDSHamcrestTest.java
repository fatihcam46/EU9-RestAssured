...
//http://54.145.124.192:1000/ords/hr/employees?q={"job_id": "IT_PROG"}
        given().accept(ContentType.JSON)//start with given()
                .and().queryParam("q","{\"job_id\": \"IT_PROG\"}")//From postman copy
        .when()
                .get("/employees")
        .then()
                .statusCode(200)
                .body("items.job_id",everyItem(equalTo("IT_PROG")))//every item has? equalTo  IT PROG
                .body("items.first_name",containsInRelativeOrder("Alexander","Bruce","David","Valli","Diana"))//it has to same order
                //contains with order//to get names directly:Alexander","Bruce","David","Valli","Diana
    ...
    public void employeesTest2(){
        ...
        .when()
         ...
         .then()
                ...
                .extract().jsonPath();//extract(): it will return
        //extract() --> method that allow us to get response object after we use then() method.
        //assert that we have only 5 firstnames
        assertThat(jsonPath.getList("items.first_name"),hasSize(5));//HamcrestMatchers method assertThat
        ...
        //asserThat method:without print
      //  jsonPath.prettyPrint();  print all
